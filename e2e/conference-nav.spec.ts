...
test.describe('Conference nav', () => {
  ...
  test('conferences are sorted with active first', async ({ page }) => {
    const items = page.locator('.conference-nav__item');
    const count = await items.count();
    if (count === 0) {
      test.skip();
    ...
    // All items should be clickable (no pointer-events: none)
    const first = items.first();
    const pointerEvents = await first.evaluate(el => getComputedStyle(el).pointerEvents);
    expect(pointerEvents).not.toBe('none');

    // No coming-soon classes should exist
    await expect(page.locator('.conference-nav__item--coming-soon')).toHaveCount(0);
  });
});
