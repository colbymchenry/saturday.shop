...
test.describe('Search page', () => {
  ...
  test('search returns product results in a grid', async ({ page }) => {
    ...
    const cards = grid.locator('.search-card');
    expect(await cards.count()).toBeGreaterThanOrEqual(1);
  });

  test('product cards have image, title, and price', async ({ page }) => {
    await page.goto('/search?q=a');
    const card = page.locator('.search-card').first();
    await expect(card.locator('.search-card__media img')).toBeVisible();
    await expect(card.locator('.search-card__title')).not.toBeEmpty();
    await expect(card.locator('.search-card__current-price')).not.toBeEmpty();
  });
